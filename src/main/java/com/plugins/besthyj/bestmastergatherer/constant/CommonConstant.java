package com.plugins.besthyj.bestmastergatherer.constant;

import com.plugins.besthyj.bestmastergatherer.util.ColorUtil;

public class CommonConstant {
    public static final String PLUGIN_NAME = "BestMasterGatherer";
    public static final String PLUGIN_NAME_PREFIX = ColorUtil.translateColorCode("&7[&6收集系统&7] &8>> ");

    public static final String COLLECT_FOLDER = "collectGUI";
    public static final String ATTRIBUTE_FOLDER = "attributeGUI";

    public static final String COLLECTED = ColorUtil.translateColorCode("&a√&a已收集");
    public static final String UNCOLLECTED = ColorUtil.translateColorCode("&7×&7未收集");
}
